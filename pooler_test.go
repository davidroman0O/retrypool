...
func TestCustomTimer(t *testing.T) {
	...
	if pool.config.timer != customTimer {
...
func TestDeadTaskErrorsAndDurations(t *testing.T) {
	...
	// Check totalDuration
	expectedMinDuration := 300 * time.Millisecond // At least two processing times + one delay
...
// TestPutTaskAsDead tests manually marking a task as dead
func TestPutTaskAsDead(t *testing.T) {
	ctx := context.Background()
	worker := &FlakyWorker{failuresLeft: 5}
	pool := New(ctx, []Worker[TaskInt]{worker})
	...
	err = pool.PutTaskAsDead(task)
	if err != nil {
		t.Fatalf("Failed to mark task as dead: %v", err)
	}
	...
	if deadTasks[0].Data.Data != 1 {
		t.Errorf("Expected dead task data to be 1, got %d", deadTasks[0].Data.Data)
	}
}

// TestDispatchDeadTask tests re-dispatching a dead task
func TestDispatchDeadTask(t *testing.T) {
	ctx := context.Background()
	worker := &FlakyWorker{failuresLeft: 1}
	pool := New(ctx, []Worker[TaskInt]{worker})
	...
	// Wait for the task to become dead
	pool.WaitWithCallback(ctx, func(queueSize, processingCount int) bool {
		return queueSize > 0 || processingCount > 0
	}, 10*time.Millisecond)
	...
	// Re-dispatch the dead task
	err = pool.DispatchDeadTask(task)
	if err != nil {
		t.Fatalf("Failed to re-dispatch dead task: %v", err)
	}

	// Wait for the re-dispatched task to complete
	pool.WaitWithCallback(ctx, func(queueSize, processingCount int) bool {
		return queueSize > 0 || processingCount > 0
	}, 10*time.Millisecond)
	...
	// Check that there are no dead tasks now
	deadTasks = pool.DeadTasks()
	if len(deadTasks) != 0 {
		t.Errorf("Expected 0 dead tasks after re-dispatching, got %d", len(deadTasks))
	}
}

// TestRemoveDeadTask tests removing a dead task
func TestRemoveDeadTask(t *testing.T) {
	ctx := context.Background()
	worker := &FlakyWorker{failuresLeft: 5}
	pool := New(ctx, []Worker[TaskInt]{worker},
		WithAttempts[TaskInt](1),
	)
	...
	err = pool.RemoveDeadTask(task)
	if err != nil {
		t.Fatalf("Failed to remove dead task: %v", err)
	}

	// Check that there are no dead tasks now
	deadTasks = pool.DeadTasks()
	if len(deadTasks) != 0 {
		t.Errorf("Expected 0 dead tasks after removal, got %d", len(deadTasks))
	}
}
